...
import { 
  ...
  VisibilityOutlined, RefreshOutlined, SchoolOutlined, PersonAddOutlined, 
  AccountTreeOutlined, NavigateNext, HistoryToggleOffOutlined, 
  GridViewOutlined, ViewListOutlined, CheckCircleOutline, FileDownloadOutlined, 
  CloseOutlined, BadgeOutlined
...
const CACHE_KEY = "ACADEMIC_STAFF_VAULT";
...
const AcademicStaffManager = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const fetchedData: AcademicStaff[] = await response.json();
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/academic-staff/${targetId}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
      triggerSnackbar("Staff record successfully purged", "success");
  ...
  const exportToCSV = () => {
    const headers = ["ID,Name,Designation,Department,JoinedDate\n"];
    const rows = filteredData.map(item => 
      `${item._id},"${item.name}","${item.jobDescription}","${item.department}",${item.createdAt}`
    );
    const blob = new Blob([...headers, rows.join("\n")], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `academic-staff-directory.csv`;
    a.click();
    triggerSnackbar("CSV Directory Downloaded", "success");
    setHistory(prev => [`Exported Staff Directory CSV`, ...prev].slice(0, 8));
  };
  ...
  // View Renders
  if (showAddForm) return <ThemeProvider theme={montserratTheme}><CreateAcademicStaff onBack={() => { setShowAddForm(false); fetchData(); }} /></ThemeProvider>;
  if (editingItem) return <ThemeProvider theme={montserratTheme}><UpdateAcademicStaff itemData={editingItem} onBack={() => { setEditingItem(null); fetchData(); }} /></ThemeProvider>;
      ...
      <Box sx={{ width: "100%", minHeight: "100vh", bgcolor: "#F4F7FA", p: { xs: 1.5, md: 3 } }}>
          ...
          <Box sx={{ width: { xs: '100%', md: '66.66%' } }}>
            <Paper elevation={0} sx={{ p: 1, px: 2, borderRadius: "8px", border: "1px solid #E2E8F0", display: 'flex', alignItems: 'center', gap: 2, height: '100%' }}>
              <TextField
                ...
                size="small"
                variant="standard"
                placeholder="Search by name, department, or academic title..."
                value={searchQuery}
                    ...
                    <motion.div key="table-view" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
                        ...
                        <Table size="medium" sx={{ minWidth: 900 }}>
                            ...
                            <TableRow>
                                ...
                                <TableCell sx={{ fontWeight: 800, fontSize: "0.7rem", color: "#64748B", letterSpacing: 0.5 }}>PHOTO</TableCell>
                                <TableCell sx={{ fontWeight: 800, fontSize: "0.7rem", color: "#64748B", letterSpacing: 0.5 }}>STAFF DETAILS</TableCell>
                                <TableCell sx={{ fontWeight: 800, fontSize: "0.7rem", color: "#64748B", letterSpacing: 0.5 }}>DEPARTMENT</TableCell>
                                <TableCell sx={{ fontWeight: 800, fontSize: "0.7rem", color: "#64748B", letterSpacing: 0.5 }}>ACADEMIC TITLE</TableCell>
                                <TableCell sx={{ fontWeight: 800, fontSize: "0.7rem", color: "#64748B", letterSpacing: 0.5 }}>JOINED</TableCell>
                                ...
                                <TableRow key={item._id} hover sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                    ...
                                    <TableCell>
                                        <Chip 
                                          ...
                                          sx={{ fontWeight: 700, fontSize: '0.65rem', bgcolor: "#F0F5F6", color: PRIMARY_TEAL }} 
                                        />
                                    </TableCell>
                                    <TableCell>
                                    ...
                                    <Card sx={{ borderRadius: '12px', border: '1px solid #E2E8F0', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', transition: 'transform 0.2s', '&:hover': { transform: 'translateY(-4px)', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)' } }}>
                                        ...
                                        <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                                            <Stack direction="row" alignItems="center" spacing={1} mb={2}>
                                                <AccountTreeOutlined sx={{ fontSize: 16, color: PRIMARY_TEAL }} />
          ...
          {viewingItem && (
            <Box>
              <Box sx={{ p: 3, bgcolor: '#F8FAFC', borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                 <Box>
                    <Typography variant="h6" sx={{ fontWeight: 800, color: PRIMARY_TEAL, fontSize: '1.2rem', letterSpacing: -0.5 }}>Staff Profile</Typography>
              ...
              <DialogContent sx={{ p: 4, maxHeight: '70vh', overflowY: 'auto' }}>
                 <Box sx={{ textAlign: 'center', mb: 4 }}>
                    ...
                    <Stack direction="row" spacing={1.5} justifyContent="center" sx={{ mt: 2 }}>
                        <Chip icon={<AccountTreeOutlined sx={{ fontSize: '1rem !important' }}/>} label={viewingItem.department} sx={{ fontWeight: 700, bgcolor: "#F0F5F6", color: PRIMARY_TEAL }} />
        ...
        >
          <Box textAlign="center" p={3}>
            ...
            <Typography sx={{ fontWeight: 800, color: "#1E293B", fontSize: '1.1rem', mb: 1 }}>Confirm Staff Purge</Typography>
            <Typography sx={{ color: "#64748B", mb: 4, fontWeight: 500, fontSize: '0.85rem', lineHeight: 1.5 }}>
              You are about to permanently delete this academic staff member from the university database. This action cannot be undone.
            </Typography>
