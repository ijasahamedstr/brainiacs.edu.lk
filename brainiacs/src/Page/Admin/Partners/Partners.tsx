...
import { 
  ...
  VisibilityOutlined, AddBusinessOutlined, BusinessOutlined, LanguageOutlined,
  DescriptionOutlined, NavigateNext, HistoryToggleOffOutlined, GridViewOutlined, 
  ViewListOutlined, CheckCircleOutline, FileDownloadOutlined, CloseOutlined
...
const CACHE_KEY = "PARTNER_MANAGER_VAULT";
...
const PartnerManager = () => {
  ...
  // ✅ FIXED STATE: Unified delete dialog state
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const fetchedData: Partner[] = await response.json();
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/partners/${targetId}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
      triggerSnackbar("Partner successfully purged", "success");
  ...
  const handleBulkDelete = async () => {
    ...
    setSelected([]);
    triggerSnackbar(`Purging ${count} partners...`, "success");
  ...
  const exportToCSV = () => {
    const headers = ["ID,PartnerName,Website,JoinedDate\n"];
    const rows = filteredData.map(item => 
      `${item._id},"${item.name}","${item.websiteUrl || 'N/A'}",${new Date(item.createdAt).toLocaleDateString()}`
    );
    const blob = new Blob([...headers, rows.join("\n")], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `partner-directory.csv`;
    a.click();
    triggerSnackbar("CSV Directory Downloaded", "success");
    setHistory(prev => [`Exported Partner Directory CSV`, ...prev].slice(0, 8));
  };
  ...
  // View Renders
  if (showAddForm) return <ThemeProvider theme={montserratTheme}><CreatePartner onBack={() => { setShowAddForm(false); fetchData(); }} /></ThemeProvider>;
  if (editingItem) return <ThemeProvider theme={montserratTheme}><UpdatePartner itemData={editingItem} onBack={() => { setEditingItem(null); fetchData(); }} /></ThemeProvider>;
      ...
      <Box sx={{ width: "100%", minHeight: "100vh", bgcolor: "#F4F7FA", p: { xs: 1.5, md: 3 } }}>
        ...
        <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'flex-end' }} mb={3} spacing={2}>
          <Box>
            <Breadcrumbs separator={<NavigateNext sx={{ fontSize: '0.8rem' }} />} sx={{ mb: 0.5 }}>
              ...
              <Typography color="text.primary" sx={{ fontSize: '0.65rem', fontWeight: 800, color: PRIMARY_TEAL }}>CORPORATE RELATIONS</Typography>
            </Breadcrumbs>
          ...
          <Stack direction="row" spacing={1.5} alignItems="center">
            ...
            >
             ...
             <Button 
              ...
              startIcon={<AddBusinessOutlined fontSize="small" />}
                    ...
                    <motion.div key="table-view" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
                        ...
                        <Table size="medium" sx={{ minWidth: 900 }}>
                            ...
                            <TableRow>
                                ...
                                <TableCell sx={{ fontWeight: 800, fontSize: "0.7rem", color: "#64748B", letterSpacing: 0.5 }}>LOGO</TableCell>
                                <TableCell sx={{ fontWeight: 800, fontSize: "0.7rem", color: "#64748B", letterSpacing: 0.5 }}>PARTNER INFO</TableCell>
                                <TableCell sx={{ fontWeight: 800, fontSize: "0.7rem", color: "#64748B", letterSpacing: 0.5 }}>DESCRIPTION</TableCell>
                                <TableCell sx={{ fontWeight: 800, fontSize: "0.7rem", color: "#64748B", letterSpacing: 0.5 }}>JOINED DATE</TableCell>
                            ...
                            <TableBody>
                                ...
                                <TableRow key={item._id} hover sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                    ...
                                    <TableCell>
                                        <Avatar src={item.logoUrl} variant="rounded" sx={{ width: 50, height: 50, borderRadius: "10px", border: '1px solid #E2E8F0', bgcolor: "#FFF", p: 0.5 }}>
                                    ...
                                    <TableCell>
                                        ...
                                        {item.websiteUrl ? (
                                            <Link href={item.websiteUrl} target="_blank" sx={{ display: 'flex', alignItems: 'center', gap: 0.5, textDecoration: 'none', color: '#94A3B8', fontSize: '0.7rem', mt: 0.5, fontWeight: 600 }}>
                                                <LanguageOutlined sx={{ fontSize: 14 }} /> Visit Website
                                            </Link>
                                        ) : (
                                    ...
                                    <TableCell>
                                        <Stack direction="row" spacing={1} alignItems="center">
                                            <DescriptionOutlined sx={{ fontSize: 16, color: "#64748B" }} />
                                ...
                                <Box key={item._id}>
                                    <Card sx={{ borderRadius: '12px', border: '1px solid #E2E8F0', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', transition: 'transform 0.2s', '&:hover': { transform: 'translateY(-4px)', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)' }, display: 'flex', flexDirection: 'column', height: '100%' }}>
                                        <Box sx={{ position: 'relative', p: 3, pt: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', bgcolor: '#F8FAFC', borderBottom: '1px solid #E2E8F0' }}>
                                            ...
                                            <Avatar src={item.logoUrl} variant="rounded" sx={{ width: 100, height: 100, border: '1px solid #E2E8F0', boxShadow: '0 4px 10px rgba(0,0,0,0.04)', mb: 2, bgcolor: '#FFF', p: 1 }}>
                                                <BusinessOutlined sx={{ fontSize: 40, color: '#CBD5E1' }} />
                                            </Avatar>
                                            <Typography sx={{ fontWeight: 800, fontSize: '1rem', color: '#1E293B', textAlign: 'center', lineHeight: 1.2 }}>{item.name}</Typography>
                                            {item.websiteUrl && (
                                                <Link href={item.websiteUrl} target="_blank" sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: 1, textDecoration: 'none', color: PRIMARY_TEAL, fontSize: '0.75rem', fontWeight: 600 }}>
                                                    <LanguageOutlined sx={{ fontSize: 14 }} /> Website
                                                </Link>
                                            )}
                                        </Box>
                                        <CardContent sx={{ p: 2, flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', '&:last-child': { pb: 2 } }}>
                                            <Box mb={2}>
                                                <Stack direction="row" alignItems="center" justifyContent="center" spacing={1}>
                                                    <DescriptionOutlined sx={{ fontSize: 16, color: '#64748B' }} />
                                                    <Typography sx={{ fontSize: '0.75rem', color: '#64748B', fontWeight: 600 }}>
                                                        {item.description.length} Detail Blocks
                                                    </Typography>
                                                </Stack>
                                            </Box>
                                            <Box>
          ...
          {viewingItem && (
            <Box>
              <Box sx={{ p: 3, bgcolor: '#F8FAFC', borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                 <Box>
                    <Typography variant="h6" sx={{ fontWeight: 800, color: PRIMARY_TEAL, fontSize: '1.2rem', letterSpacing: -0.5 }}>Partner Profile</Typography>
              ...
              <DialogContent sx={{ p: 4, maxHeight: '70vh', overflowY: 'auto' }}>
                 <Box sx={{ textAlign: 'center', mb: 4 }}>
                    <Avatar 
                      ...
                      variant="rounded"
                      sx={{ width: 140, height: 140, mx: 'auto', mb: 2, borderRadius: "24px", border: `1px solid #E2E8F0`, bgcolor: '#FFF', p: 1, boxShadow: '0 10px 15px -3px rgba(0,0,0,0.05)' }} 
                    />
                    <Typography variant="h4" sx={{ fontWeight: 900, color: PRIMARY_TEAL, mb: 1, letterSpacing: -0.5 }}>{viewingItem.name}</Typography>
                    
                    {viewingItem.websiteUrl && (
                        <Stack direction="row" justifyContent="center" sx={{ mt: 2 }}>
                            <Button 
                                variant="outlined" 
                                href={viewingItem.websiteUrl} 
                                target="_blank"
                                startIcon={<LanguageOutlined />}
                                sx={{ borderRadius: '8px', color: PRIMARY_TEAL, borderColor: PRIMARY_TEAL, fontWeight: 700 }}
                            >
                 ...
                 <Paper elevation={0} sx={{ p: 4, bgcolor: "#F8FAFC", borderRadius: "16px", border: "1px solid #E2E8F0" }}>
                    ...
                    {viewingItem.description.map((text, i) => (
                        <Typography key={i} sx={{ color: "#475569", lineHeight: 1.8, fontSize: "0.95rem", mb: i < viewingItem.description.length - 1 ? 2 : 0 }}>
                            {text}
                        </Typography>
                    ))}
                 </Paper>
        ...
        >
          <Box textAlign="center" p={3}>
            ...
            <Typography sx={{ fontWeight: 800, color: "#1E293B", fontSize: '1.1rem', mb: 1 }}>Confirm Partner Purge</Typography>
            <Typography sx={{ color: "#64748B", mb: 4, fontWeight: 500, fontSize: '0.85rem', lineHeight: 1.5 }}>
              You are about to permanently delete this partner and their logo from the directory. This action cannot be undone.
            </Typography>
