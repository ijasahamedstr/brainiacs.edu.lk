...
export const createStudent = async (req, res) => {
  try {
    // 1. Check if the field exists before parsing
    if (!req.body.data) {
      return res.status(400).json({ 
        success: false, 
        message: "No data field found in request. Ensure frontend appends to 'data'." 
      });
    }

    // 2. Parse the JSON payload (Must match the frontend key 'data')
